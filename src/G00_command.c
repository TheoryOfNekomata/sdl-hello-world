...
int G00_RuntimeExecuteCommand(struct G00_CommandEntry G00_COMMAND_ENTRIES[], char command[255], char args[255], struct G00_App* app) {
	unsigned int i = 0;
	do {
		if (!strcmpi(command, G00_COMMAND_ENTRIES[i].name)) {
			if (!G00_COMMAND_ENTRIES[i].execute_fn) {
				fprintf(stdout, "WRN003: Command \"%s\" not yet implemented, performing noop\n", command);
				return 1;
			}
			...
			if (command_result < 0) {
				fprintf(stderr, "ERR002: Command \"%s\" error (return code %d)\n", command, command_result);
			} else if (command_result > 0) {
				fprintf(stdout, "WRN002: Command \"%s\" warning (return code %d)\n", command, command_result);
			}
	...
	fprintf(stderr, "ERR001: Unknown command \"%s\"\n", command);
	return -1;
}
